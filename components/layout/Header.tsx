    ...
    <>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
           ...
           {/* Menu Content */}
           <div className="fixed inset-y-0 left-0 w-3/4 max-w-xs bg-white shadow-xl transform transition-transform duration-300 ease-in-out">
              <div className="flex items-center justify-between px-4 py-6 border-b border-zinc-100">
              ...
              <nav className="flex flex-col p-4 space-y-2">
                 <Link 
                    href="/catalog" 
                    className="px-4 py-3 text-lg font-medium text-zinc-900 hover:bg-zinc-50 rounded-sm"
                    onClick={() => setIsMobileMenuOpen(false)}
                 >
                    Каталог
                 </Link>
                 ...
                 <div className="border-t border-zinc-100 my-2 pt-2">
                    <Link 
                       href={user ? "/profile" : "/login"} 
                       className="px-4 py-3 text-lg font-medium text-zinc-900 hover:bg-zinc-50 rounded-sm flex items-center gap-2"
                       onClick={() => setIsMobileMenuOpen(false)}
                    >
                       <User className="h-5 w-5" />
                       {user ? 'Профиль' : 'Войти'}
